...
export async function getPageData(params, locale, preview) {
  const { slug: s, global } = params;
  const slug = s.join("/");
  // Find the pages that match this slug
  //
  const pagesData = await fetchAPI(
    `/pages?slug=${slug}&_locale=${locale}&global=${global}&status=published${
...
export async function getGlobalData(locale, id = 1) {
  const global = await fetchAPI(`/globals?id=${id}&_locale=${locale}`);
  return global[0];
}
